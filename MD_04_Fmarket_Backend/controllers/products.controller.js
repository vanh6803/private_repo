...
const detailProduct = async (req, res, next) => {
  try {
    ...
    // Fetching options for the product
...
const getTopProduct = async (req, res, next) => {
  try {
    const topProducts = await optionModel.option.aggregate([
      {
        $group: {
          _id: "$product_id",
      ...
      {
        $lookup: {
          from: "products", // Tên của collection sản phẩm trong database của bạn
          localField: "_id",
          foreignField: "_id",
          as: "product",
        },
    ...
    return res.status(200).json({
      code: 200,
      result: topProducts,
      message: "Get top 10 products successfully",
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ code: 500, message: error.message });
